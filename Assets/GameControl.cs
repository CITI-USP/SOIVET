...
public class GameControl : MonoBehaviour {
	...
	private void Success(){
		...
		//GameObject.Find ("led0").GetComponent<Renderer>().enabled = true;
		...
		try{			
			...
			GameObject.Find ("led" + colliderCount).GetComponent<Renderer> ().enabled = true;

		}catch(System.Exception e){			
			Debug.Log ("ERROR led not found: " + "led" + colliderCount);
		}

		if( (SceneManager.GetActiveScene().name.Equals("1.Labirinto_fase1") && colliderCount == 4) ||
			(SceneManager.GetActiveScene().name.Equals("2.Labirinto_fase2") && colliderCount == 32) )
		{
			Finish ();
		} else {
	...
	void Start () {	
		...
		if (ApplicationManager.currentLevel == 0) {
			...
			GameObject.Find ("mapaDurex").SetActive (false);
	...
	void FixedUpdate () {
		...
		//Menu
		if(Input.GetButtonDown("Fire2") || Input.GetButtonDown("Fire3")  || Input.GetMouseButtonDown(0) )			
		...
		if (failed) {
			...
			transform.position = Vector3.SmoothDamp(transform.position, path.Pop(), ref velocity, .3f);
			transform.rotation = Quaternion.Slerp (transform.localRotation, pathRotation.Pop (), .3f);
